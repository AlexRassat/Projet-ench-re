...
public class ModificationProfileServlet extends HttpServlet {
 ...
 //       int id = (int)session.getAttribute("id");
        ...
        try {
            user = profilUser.selectById(2);
        } catch (BusinessException e) {
            // TODO Auto-generated catch block
        }
        request.setAttribute("profil", user); 
		getServletContext().getRequestDispatcher("/WEB-INF/jsp/ModificationProfil.jsp").forward(request, response);
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int id = (int)session.getAttribute("id");
	        ...
	        // Je récupere les nouvelles données de l'utilisateur
